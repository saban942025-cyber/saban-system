// public/js/chatbot-engine.js
import { SabanPush } from './notifications.js';
import { SabanSounds } from './sounds.js';

export class SabanChatbot {
    constructor(db, userContext) {
        this.db = db;
        this.user = userContext; 
        
        this.rules = [
            // --- טריגר חירום: שינוי הזמנה (הכי חשוב!) ---
            {
                keywords: ["להוסיף", "לשנות", "להגדיל", "טעות", "עצור", "שינוי הזמנה"],
                answer: "🛑 עצרתי את הטיפול בהזמנה! \nתייגתי את המחסנאי (אורן) ואת רמי למקרה דחוף.\nאנא רשום כאן מה בדיוק להוסיף/לשנות?",
                action: "stop_order_alert"
            },
            // --- זיהוי הזמנות ---
            {
                keywords: ["הזמנה", "להזמין", "תשלח", "מלט", "חול", "בלות", "טיט", "דבק", "גבס", "בלוקים", "ברזל"],
                answer: "קיבלתי את הרשימה! 📝\nאני מעביר אותה מיד לצוות (רמי/יואב) להקלדה בקומקס.\nמספר הזמנה וקובץ PDF ישלחו אליך לאישור ברגע שזה יהיה מוכן.",
                action: "order_received"
            },
            // --- לוגיסטיקה ---
            {
                keywords: ["מנוף", "קומה", "גג", "להרים", "משאית"],
                answer: "אין בעיה, נספק עם מנוף (חכמת/אמיר). 🏗️\nרק תוודא שאין חוטי חשמל בגישה ושמשטח הפריקה פנוי.",
                buttons: [{ label: "מאשר גישה תקינה ✅", payload: "crane_ok" }]
            },
            // --- חירום ---
            {
                keywords: ["דחוף", "בהול", "עכשיו", "תקוע", "תקלה", "לא הגיע"],
                answer: "הבנתי שזה דחוף! 🚨\nהקפצתי התראה מיוחדת למנהל התפעול. איתך תוך דקות.",
                action: "urgent_alert"
            },
            // --- בירור זמנים ---
            {
                keywords: ["מתי", "צפי", "מגיע", "שעה"],
                answer: "המשאית בסידור עבודה. 🚚\nאני בודק מיקום מול הנהג (GPS) ושולח לך עדכון מיד.",
                action: "check_eta"
            },
            // --- היתרים ---
            {
                keywords: ["הרצליה", "פיתוח", "תל אביב"],
                answer: "🛑 שים לב: באזור הזה העירייה דורשת היתר כניסה/העמדה.\nיש לך היתר בתוקף?",
                buttons: [{ label: "כן, יש לי 👍", payload: "permit_yes" }, { label: "לא, תבדקו לי ❓", payload: "permit_no" }]
            },
            // --- כללי ---
            {
                keywords: ["היי", "שלום", "בוקר טוב", "ערב טוב", "אהלן"],
                answer: "אהלן {name}! כאן הבוט של סבן. 👋\nאפשר להקליד כאן הזמנה, לבקש שינוי, או לשאול מתי מגיע.",
                buttons: [{ label: "הדבק הזמנה", payload: "paste_order" }]
            }
            
            {
    keywords: ["היסטוריה", "הזמנות שלי", "מה הזמנתי", "LOG"],
    answer: "בטח! 📜\nאתה יכול לראות את כל ההזמנות הקודמות שלך באזור האישי, או לבקש ממני תאריך ספציפי.\nהנה ההזמנה האחרונה שסופקה: [שליפת נתונים...]",
    action: "fetch_history"
},
        ];
    }

    async ask(question) {
        if (!question) return null;
        const cleanQ = question.toLowerCase();

        for (const rule of this.rules) {
            const match = rule.keywords.some(kw => cleanQ.includes(kw));
            
            if (match) {
                console.log("✅ Bot Match:", rule.keywords[0]);
                
                // טיפול בסאונד והתראות
                if (rule.action === 'stop_order_alert' || rule.action === 'urgent_alert') {
                    SabanSounds.playAlert(); // 🔊 צפצוף חזק
                    // שליחת התראות למנהלים
                    await SabanPush.send('admin_rami', '🛑 עצור ליקוט!', `הלקוח ${this.user.name} ביקש שינוי דחוף!`);
                } else {
                    SabanSounds.playMessage(); // 🎵 צליל רגיל
                }

                const finalText = rule.answer.replace("{name}", this.user.name || "חבר");
                
                return { 
                    text: finalText, 
                    buttons: rule.buttons || [],
                    action: rule.action || null
                };
            }
        }

        return { 
            text: "קיבלתי. ההודעה הועברה לנציג אנושי להמשך טיפול. 👨‍💻",
            action: "fallback"
        };
    }
}

